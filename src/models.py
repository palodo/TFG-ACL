...
class SimpleCNNSelector(nn.Module):
    ...
    def __init__(self, checkpoint_path, device='cuda:0'):
        ...
        if has_classifier_only and not has_backbone_prefix:
            print(f"    Detectado formato de checkpoint sin prefijo 'backbone.', remapeando keys...")
            remapped_state_dict = {}
        ...
        if missing_keys:
            print(f"    Keys faltantes en checkpoint: {missing_keys[:3]}...")
        if unexpected_keys:
            print(f"  ℹ  Keys inesperadas en checkpoint: {unexpected_keys[:3]}...")
